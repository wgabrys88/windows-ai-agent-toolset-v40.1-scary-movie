...
_s(_u32, "mouse_event", [W.DWORD, W.DWORD, W.DWORD, W.DWORD, ctypes.c_ulong], None)
...
_RE_OBS = re.compile(r'(?:observation|phenomenology)\s*[:=]\s*["\']?(.*?)["\']?\s*(?=[,}\n]|$)', re.I | re.S)
_RE_BBOX = re.compile(r'(?:bbox|box|B\d)\s*[$:$$]\s*(\d{1,4})\s*[,;]\s*(\d{1,4})\s*[,;]\s*(\d{1,4})\s*[,;]\s*(\d{1,4})\s*[$$$]?', re.I)
_RE_ACTION = re.compile(r'(click|right_click|double_click|drag|move)\s*$\s*(\d{1,4})\s*[,;]\s*(\d{1,4})(?:\s*[,;→\->]+\s*(\d{1,4})\s*[,;]\s*(\d{1,4}))?\s*$', re.I)
_RE_UCI = re.compile(r'\b([a-h][1-8][a-h][1-8][qrbn]?)\b', re.I)
_RE_COORDS = re.compile(r'(?:at|position|pos|to|from)?\s*$?\s*(\d{2,4})\s*[,;]\s*(\d{2,4})\s*$?', re.I)
